...
mean_scores <- read_rds(exp$result("scores.rds"))

plot_part_A <-
  ggplot(mean_scores$summ, aes(paste0(method_id, "\n", params_id), cor)) +
  ggbeeswarm::geom_quasirandom(aes(color = backbone_name, shape = difficulty), size = 3)  +
  theme_bw() +
  scale_x_discrete("") +
...
plot_part_C <- pmap(design_velocity_oi, function(dataset_id, method_id, params_id, ...) {
  ...
  dynplot_dimred(dataset) +
    geom_cell_point(aes(color = transform_groundtruth_velocity(velocity$velocity_vector[,feature_oi]))) +
