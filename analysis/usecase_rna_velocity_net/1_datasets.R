...
pwalk(design_datasets, function(id, seed, backbone_name) {
  if (!file.exists(exp$dataset_file(id))) {
    ...
    # np <- as.matrix(dataset$net_propensity)
    # lpr <- as.matrix(dataset$log_propensity_ratios)
    # qplot(as.vector(np), as.vector(lpr)) + labs(x = "Net propensity", y = "Log propensity ratios")
    gc()
  }
})
...
pwalk(design_datasets, function(id, seed, backbone_name) {
  ...
  out <- dyngen:::.generate_cells_compute_log_propensity_ratios(model, model$simulations$reaction_propensities)
  model$simulations$log_propensity_ratios <- out$ratios
  model$simulations$net_propensity <- out$diff

  lpre <- out$ratios[model$experiment$cell_info$step_ix, ]
  pde <- out$diff[model$experiment$cell_info$step_ix, ]
  rownames(lpre) <- rownames(pde) <- model$experiment$cell_info$cell_id
  model$experiment$log_propensity_ratios <- lpre
  model$experiment$net_propensity <- pde

  dataset$log_propensity_ratios <- lpre
  dataset$net_propensity <- pde
