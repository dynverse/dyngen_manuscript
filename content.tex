...
\begin{table}[h]
	\caption{\textbf{Reactions affecting the abundance levels of pre-mRNA $w_G$, mRNA $x_G$ and proteins $y_G$ of gene $G$.} Define the set of regulators of $G$ as $R_G$, the set of upregulating regulators of $G$ as $R^+_G$, and the set of downregulating regulators of $G$ as $R^-_G$. Parameters used in the propensity formulae are defined in Table \ref{tab:reaction_params}.} \label{tab:reaction_def}
	\centering
	\begin{tabular}{|lcc|}
		\hline
		Reaction & Effect & Propensity \\ \hline \hline
		Transcription & $\emptyset \rightarrow w_G$ & $wpr_G \times \frac{ba_G - 1 + \prod\limits_{H \in R^+_G}(1 + reg_{G,H})}{ba_G - 1 + \prod\limits_{H \in R_G}(1 + reg_{G,H})}$, \\
%		& & with $reg_{G,H} = \left(str_{G,H} \times \frac{y_H}{0.5 \times maxy_H}\right) ^ {co_{G,H}}$ \\
%		& & and $maxy_H = \frac{wpr_H \times ypr_H}{xdr_H \times ydr_H}$ \\
		...
		Protein degradation & $y_G \rightarrow \emptyset$ & $ydr_G \times y_G$ \\ \hline
	\end{tabular}
\end{table}
...
% are independant and result in multiplicative effect.

% defining P_H is the probability that H is bound to G, 
% and the number of molecules produced C_H = ifelse(eff_G,H == 1, wpr_G × str_{G,H}, 0)
% should the expented no. of reactions (propensity) not be defined as:
% BA_G + Prod_H {C_H × P_H + (1 - P_H)} - Prod_H {1 - P_H} ?
...
% max protein doesn't work as expected (given any simulation in model2):
%df <- data.frame(time = model2$simulations$meta$sim_time, as.matrix(model2$simulations$counts)) %>% 
%  gather(mol, value, -time) %>% 
%  filter(grepl("^y_", mol)) %>% 
%  mutate(feature_id = gsub("^y_", "", mol)) %>% 
%  left_join(model2$feature_info %>% transmute(feature_id, max_protein = wpr / xdr * ypr / ydr, module_id), by = "feature_id") %>% 
%  mutate(tfname = gsub("^[^_]*_", "", feature_id))
%ggplot(df) + geom_line(aes(time, value / max_protein, colour = tfname, group = feature_id)) + facet_wrap(~module_id)
...
\begin{table}[h]
	...
	\begin{tabular}{|lrl|}
		...
		Transcription rate & $wpr_G$ & $\in \{. | . \in N(100, 20), . \geq 10\}$ \\
		Splicing rate & $wsr_G$ & $\in \{. | . \in N(10, 2), . \geq 2\}$ \\
		mRNA degradation rate & $xdr_G$ & $\in \{. | . \in N(5, 1), . \geq 2\}$ \\
		Translation rate & $ypr_G$ & $\in \{. | . \in N(5, 1), . \geq 2\}$ \\
		Protein degradation rate & $ydr_G$ & $\in \{. | . \in N(3, 0.5), . \geq 1\}$ \\
		Interaction strength & $str_{G,H}$ & $\in \{10^. | . \in U(0, 2)\}$ * \\ 
		Interaction cooperativity & $co_{G,H}$ & $\in U(0.5, 2)$ * \\\hline\hline
		...
		Basal expression & $ba_G$ & $= \begin{cases} 1 & \mbox{if } R^+_G = \emptyset \\ 0.0001 & \mbox{if } R^-_G = \emptyset \mbox{ and } R^+_G \neq \emptyset \\ 0.5 & \mbox{otherwise} \end{cases}$ * \\ \hline
