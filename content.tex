...
\comment{GRN is defined nowhere}
...
\comment{could use figure; one with GRNs of different topologies, another with rewiring events}
...
\comment{explain what is meant by 'sampling' and 'profiling technique' a little better}
...
\comment{show that dyngen output resembles real data using e.g. countsimQC?}

\comment{add result figures pertaining use cases}
...
\comment{also adding more types of molecules, e.g. protein complex, small rnas, PPI}
\section{Methods}
\comment{The method section is REALLY rough at this stage.}
...
\comment{mention strength and cooperativity.}
...
\comment{Welcome to the beginning of the end}
...
\comment{TODO: add step-by-step  derivation of transcription formula}
