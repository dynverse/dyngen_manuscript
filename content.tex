...
\begin{table}[h]
	...
	\begin{tabular}{|lrl|}
		...
		Transcription rate & $\wpr{G}$ & $\in \{. | . \in N(100, 20), . \geq 10\}$ \\
		Splicing rate & $\wsr G$ & $\in \{. | . \in N(10, 2), . \geq 2\}$ \\
		mRNA degradation rate & $\xdr{G}$ & $\in \{. | . \in N(5, 1), . \geq 2\}$ \\
		Translation rate & $\ypr{G}$ & $\in \{. | . \in N(5, 1), . \geq 2\}$ \\
		Protein degradation rate & $\ydr G$ & $\in \{. | . \in N(3, 0.5), . \geq 1\}$ \\
		Interaction strength & $\str{G,H}$ & $\in \{10^. | . \in U(0, 2)\}$ * \\ 
