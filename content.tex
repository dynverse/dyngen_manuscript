...
\begin{figure}[htb!]
	...
	\caption{The workflow of dyngen.}
...
In dyngen, the behaviour of the dynamic process is determined in the way that modules are regulating each other, where each module is a set of coregulated genes. By carefully designing the module network, the desired dynamic behaviour can be achieved. A module may have basal expression, which means that pre-mRNA of the genes in this module will be transcribed without the presence of transcription factor molecules. 
Basal expression
Burn phase

Several examples of module networks are given (Figure \ref{fig:example_backbones}). A linear process can by achieved by simply chaining together several modules. A cyclic process is a chain of modules, where the last module represses the first module. A bifurcating process consists of two modules that repress each other, and some modules leading up to and resulting from the bifurcation. A converging 
Note that these examples represent the bare minimum in terms of number of modules used. Using more modules is typically desired, since some transitions will otherwise occur too quickly.
...
\subsection{Simulate developmental backbone}
\blindtext
