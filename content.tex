...
We introduce \texttt{dyngen}, a multi-modality simulator of single cells and their dynamics (Figure \ref{fig:dyngen}A). A cell is simulated using Gillespie's Stochastic Simulation Algorithm (SSA) \cite{gillespie_exactstochasticsimulation_1977} where a cell consists of a set of molecules. Throughout a simulation, reactions (e.g. transcription, splicing) modify the abundance of these molecules, and the likelihood of a reaction occurring is again dependent on molecule abundance.

By simulating a cell over time in terms of its molecular state and the reactions that are allowed to occur, the simulator is more extendible to new modalities or experimental procedures (Figure~\ref{fig:dyngen}B). We demonstrate \texttt{dyngen}'s flexibility by simulating three different types of biological experiments, and using these simulations to develop benchmarking methodologies for recently developed computational tools.
...
One of dyngen's main advantages is that depending on how the GRN is designed, different cellular developmental processes can be simulated. Different GRNs can result in branching, converging, cyclic, or even disconnected developmental topologies. Multiple simulations with slightly different GRNs can emulate rewiring events in disease or batch effects.

\mycomment{could use figure; one with GRNs of different topologies, another with rewiring events}
...
The final main advantage is that \texttt{dyngen} can be used to simulate different experimental conditions. By default, dyngen supports snapshot experiments (uniformly sampling from an asynchronous dynamic process) and time-series experiments (sampling cells from different intervals in the simulation). 
It is possible to implement other experimental protocols, such as sampling the same cell at regular intervals or performing perturbation experiments. 

\mycomment{add result figures pertaining use cases}
...
%\mycomment{also adding more types of molecules, e.g. protein complex, small rnas, PPI}
...
\mycomment{mention strength, cooperativity, effect and independence factor.}
...
\texttt{dyngen} uses Gillespie's Stochastic Simulation Algorithm (SSA) to simulate dynamic processes. An SSA simulation is an iterative process where at each iteration one reaction is triggered. 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{fig/example/bb_linear/plot} 
	\caption{}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{fig/example/bb_bifurcating/plot} 
	\caption{}
	\label{fig:example_bifurcating}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{fig/example/bb_cycle/plot} 
	\caption{}
...
%\begin{figure}[H]
%	\centering
%	\includegraphics[width=\linewidth]{fig/example/bb_binary_tree/plot} 
%	\caption{}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{fig/example/bb_converging/plot} 
	\caption{}
	\label{fig:example_converging}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{fig/example/bb_bifurcating_converging/plot} 
	\caption{}
	\label{fig:example_bifurcating_converging}
\end{figure}
...
%\begin{figure}[H]
%	\centering
%	\includegraphics[width=\linewidth]{fig/example/bb_bifurcating_cycle/plot} 
%	\caption{}
%	\label{fig:example_bifurcating_cycle}
%\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{fig/example/bb_bifurcating_loop/plot} 
	\caption{}
	\label{fig:example_bifurcating_loop}
\end{figure}
...
%\begin{figure}[H]
%	\centering
%	\includegraphics[width=\linewidth]{fig/example/bb_disconnected/plot} 
%	\caption{}
%	\label{fig:example_disconnected}
%\end{figure}
