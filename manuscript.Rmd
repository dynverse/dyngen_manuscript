...
**Purpose:** When developing new types of tools for single-cell analyses, there is often a lack of datasets on which to quantitatively assess the performance.A common problem of pioneering computational tools is that during their development, there are rarely sufficient datasets available for adequately quantitatively assessing its performance.  
**Results:** We developed dyngen, a multi-modality simulator of single cells. In dyngen, the biomolecular state of an _in silico_ cell changes over time according to a predefined gene regulatory network.  We used dyngen to benchmark three upcoming methods for single-cell analyses: RNA velocity, casewise network inference and trajectory alignment methods. The simulator is easily extendible for adding new modalities or experimental procedures. We demonstrate dyngen's flexibility by applying it to benchmark RNA velocity, casewise network inference and trajectory alignment methods. 
...
However, a limitation of existing scRNA-seq profiles generators is that they would require significant methodological alterations to add additional modalities or experimental conditions., thereby limiting the applicability of each generator.


```{r generators, echo=FALSE, results='asis', warning=FALSE, message=FALSE}
modalities <- tribble(
  ~id, ~label,
  "premrna", "pre-mRNA expression",
  "mrna", "mRNA expression",
  "prot", "Protein expression",
  "promotor", "Promotor activity",
  "lineage", "Position in lineage tree",
  "reaction", "Reaction frequency"
) %>% mutate(type = "Modality")
applications <- tribble(
  ~id, ~label,
  "cluster", "Clustering",
  "diffexp", "Differential expression",
  "batch", "Batch effect correction",
  "linti", "Linear trajectory inference",
  "brati", "Branching trajectory inference",
  "disti", "Disconnected trajectory inference",
  "traja", "Trajectory alignment",
  "cni", "Casewise network inference",
  "rnav", "RNA velocity"
) %>% mutate(type = "Applications")
labels <- bind_rows(modalities, applications)

methods <- tribble(
  ~package, ~citation, ~modality, ~application, ~model,
  "splatter", "zappia_splattersimulationsinglecell_2017",
  c("mrna"), c("cluster", "linti", "batch", "diffexp"),
  "Several distributional models with parameters that allow the generation of datasets by sampling from distributions
Cell types, linear trajectories and batch effects can be simulated by changing the parameters of certain genes",

  "powsimR", "vieth_powsimrpoweranalysis_2017",
  c("mrna"), c("diffexp", "cluster"),
  "Negative binomial or zero-inflated negative binomial. Cell types can be simulated by changing the parameters of this distribution.",

  "PROSSTT", "papadopoulos_prossttprobabilisticsimulation_2018",
  c("mrna"), c("linti", "brati"),
  "(1) Generation of a random tree. (2) The expression of each gene is seen as a sum of contributions from different expression programs. The activity of each program depends on pseudotime. (3) Generation of cells and expression by random sampling along the trajectory, and from a negative binomial distribution.",

  "SymSim", "zhang_simulatingmultiplefaceted_2019",
  c("mrna", "promotor"), c("cluster", "linti", "brati", "disti"),
  "Kinetic model of each gene, in which it can be on or off, and where the mRNA can be degraded. The parameters of this model depend on a gene effect and cell effect, each of these effects represents a vector in an n-dimensional space (e.g. 10). The cell effect vectors are sampled in such a way that cells that have to be more similar (according to a tree provided by the user), will also have a more similar vector.",
 
  "dyngen", "",
  c("premrna", "mrna", "prot", "promotor", "lineage", "reaction"),
  c("cluster", "linti", "brati", "disti", "batch", "diffexp", "cni", "rnav", "traja"),
  "Simulation of a thermodynamic model of gene regulation, in which a set of transcription factors can bind the promoter and can have varying effects on the activity of the promoter. Clusters or trajectories are simulated by specifying a regulatory network that will induce a certain diversification of expression. Expression is simulated using a single-molecule model, after which individual UMIs are simulated using a multinomial distribution. Apart from gene regulation, splicing and protein production are also simulated."
) %>%
  mutate(
citation = ifelse(citation == "", "", paste0("\\cite{", citation, "}")),
plabel = paste0(package, " ", citation)
  )

tab <-
  full_join(
labels %>% crossing(methods) %>% select(-modality, -application),
methods %>% rowwise() %>% mutate(id = list(c(modality, application))) %>% ungroup() %>% unnest(id) %>% transmute(package, id, check = "\\ding{51}"),
by = c("id", "package")
  ) %>%
  mutate(
check = ifelse(is.na(check), "", check),
package = factor(package, levels = methods$package),
plabel = factor(plabel, levels = methods$plabel),
id = factor(id, labels$id)
  ) %>%
  arrange(match(package, methods$package), type, match(id, labels$id))
 

library(kableExtra)
tab %>%
  select(-id, -type, -model, -citation, -package) %>%
  spread(plabel, check) %>%
...
mRNA expression
Clustering
Linear trajectory inference
Batch effect correction
Differential expression
powsimR[@vieth_powsimrpoweranalysis_2017]
Negative binomial or zero-inflated negative binomial.
Cell types can be simulated by changing the parameters of this distribution
mRNA expression
Differential expression
Clustering
PROSSTT[@papadopoulos_prossttprobabilisticsimulation_2018]
(1) Generation of a random tree
(2) The expression of each gene is seen as a sum of contributions from different expression programs. The activity of each program depends on pseudotime.
(3) Generation of cells and expression by random sampling along the trajectory, and from a negative binomial distribution
mRNA expression
Linear and branching trajectory inference
SymSim[@zhang_simulatingmultiplefaceted_2019]
Kinetic model of each gene, in which it can be on or off, and where the mRNA can be degraded. The parameters of this model depend on a gene effect and cell effect, each of these effects represents a vector in an n-dimensional space (e.g. 10). The cell effect vectors are sampled in such a way that cells that have to be more similar (according to a tree provided by the user), will also have a more similar vector.
mRNA expression
promoter activity
Clustering
Linear, branching and disconnected trajectory inference
...
To demonstrate this point even further, we apply dyngen on several promising novel computational approaches for which quantitative assessment of the performance was hitherto lacking.
...
Dynamic Time Warping (DTW)[@giorgino_computingvisualizingdynamic_2009] is most commonly used to align linear trajectories. DTW is a technique originating in the field of speech recognition and aligns temporal sequences by creating a warping path between the sequences that indicates which sequence must be dilated or contracted to best match the other one. DTW can also be used in combination with smoothed pseudocells[@alpert_alignmentsinglecelltrajectories_2018]. 
...
dyngen can be used to evaluate trajectory alignment techniques. We look how well DTW can continue to distinguish between different trajectories when an increasing amount of noise is added. In Figure \ref{fig:traj_align} we can see that when noise increases, DTW can no longer distinguish trajectories clearly, and that in this case, there is no difference whether smoothed pseudocells are used or when actual cells and their gene expression are used.


\begin{figure}[htb!]
    \centering
    \includegraphics[width=.8\linewidth]{fig/traj_alignment/tares.pdf}
    \caption{
    \textbf{dyngen allows benchmarking of trajectory alignment methods.} 
  \textbf{A: Example of trajectory alignment using DTW. The segments between the two trajectories show which cells are aligned.}
\textbf{B: Shows the discrimination power of DTW decreasing as the amount of noise added increases.}
...
\begin{table}[ht]
    ...
    \begin{tabular}{|lcc|}
        ...
        Transcription & $\emptyset \rightarrow \w G$ & $\wpr G \times \frac{\ba G - \ind{G}^{|\rp{G}|} + \prod\limits_{H \in \rp{G}}(\ind G + \buf{G,H})}{\prod\limits_{H \in \rs{G}}(1 + \buf{G,H})}$ \\
...
2 datasets containing a trajectory were generated using dyngen. For each dataset, we shuffled 5\% to 75\% of the count matrix, with steps of 5\%. Next, we aligned (using DTW) all the base trajectories with all the noisy trajectories.

Let the normalized alignment distance between base trajectory x and trajectory y with z\% noise be $dxy_z$.
Then we looked at the discrimination power of DTW: how well DTW is able to distinguish between two trajectories with a certain amount of added noise. We looked at the following ratios to determine this:
\begin{equation}
    \frac{d12_{z}}{d11_{z}} \quad \forall z \in \{0.05, 0.1, 0.15, ..., 0.75 \}
\end{equation}

\begin{equation}
    \frac{d21_{z}}{d22_{z}} \quad \forall z \in \{0.05, 0.1, 0.15, ..., 0.75 \}
\end{equation}

If this ratio is high, it means that DTW is able to distinguish well between the two trajectories. If this ratio is 1, DTW is not able to distinguish between the two trajectories at all.

These datasets and ratios were generated 10 times to reduce possible stochastic effects.

## Code availability {#sec:dyngen-code}

dyngen is available as an open source R package at
\url{https://github.com/dynverse/dyngen}. All code used
in this study is made publicly available at
\url{https://github.com/rcannood/dyngen_analysis}.
