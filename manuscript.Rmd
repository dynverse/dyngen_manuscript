...
However, a limitation of scRNA-seq profiles generators is that they would require significant methodological alterations to add additional modalities or experimental conditions, thereby limiting the applicability of each generator.
...
To demonstrate this point even further, we apply dyngen on several promising novel computational approaches which have recently had a major impact on how single-cell analyses are performed but for which quantitative assessment of the performance was hitherto lacking.

## Trajectory alignment

Trajectory alignment is a technique used to discover the similarities and differences between the developmental trajectories of cells. 
This technique has been used to compare gene expression kinetics resulting from different biological processes[@cacchiarelli_aligningsinglecelldevelopmental_2018], to compare human, chimpanzee and macaque neuronal development[@kanton_organoidsinglecellgenomic_2019], to find differences in gene regulation in the presence of certain growth factors[@mcfaline-figueroa_pooledsinglecellgenetic_2019], and to compare human and mouse embryogenesis[@alpert_alignmentsinglecelltrajectories_2018].

Unfortunately, not many methods to align trajectories exist. DTW (Dynamic Time Warping)[@giorgino_computingvisualizingdynamic_2009] is most commonly used to align linear trajectories. DTW is a technique originating in the field of speech recognition and aligns temporal sequences by creating a warping path between two sequences that indicates which sequence must be dilated or contracted to best match the other one.

dyngen can be used to evaluate trajectory alignment techniques. We generated two datasets and permuted each of those datasets in five ways: we knocked down the transcription factors belonging to the end of the second milestone, we set 10% or 20% of the count matrix to zero, and we performed the combination of those techniques. This results in 12 datasets, each containing a slightly different trajectory.
We compared DTW when using smoothed pseudocells (with a gaussian kernel) and using just all cells. For each combination of trajectories, we calculated the DTW alignment. In figure x, the ratio between one trajectory with a certain permutation, and the other trajectory with the same permutation is shown. We expect this ratio to be low, as we want the alignment to be able to distinguish between different trajectories. We especially expect the smoothed versions to be able to overcome the extra 10% or 20% noise.
We performed this experiment 10 times.

In Figure \ref{fig:traj_align}, we can see that DTW with gaussian smoothing cannot distinguish between trajectory 1 and trajectory 2. When using the cells themselves, we find that they can clearly distinguish between trajectory 1 and 2 when noise is added, but not when the trajectory is limited to milestone C.
