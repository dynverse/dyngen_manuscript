...
Using a collection of transcriptomic profiles, network inference (NI) methods predict which genes are the regulators of a target gene. The output of an NI method is called a gene regulatory network (GRN, Figure\ \ref{fig:scgrn}A, top left), in which each predicted interaction can be assigned a predicted strength or effect (activating or inhibitory).

However, cells are very molecularly homogeneous, and only a fraction of all possible gene regulatory interactions is active at any point in time. The advent of single-cell omics have enabled so-called casewise network inference (CNI) methods\footnote{Other names for CNI are single-cell network inference or sample-specific network inference. Single-cell network inference (SCNI) is often used ambiguously, as classical network inference methods adapted to deal with single-cell omics data are also referred to as such. Sample-specific network inference is also ambiguous in the context of single-cell omics, as a sample is from which single cells are extracted.} to try tackling a more complex variant of the same problem, namely "Which regulators are regulating which targets in which profiles?" The end result of such methods is a single GRN for every profile.

A few CNI methods have already been developed, such as SCENIC[@aibar_scenicsinglecellregulatory_2017], LIONESS[@kuijjer_estimatingsamplespecificregulatory_2019], and SNN[@liu_personalizedcharacterizationdiseases_2016]. However, a quantitative assessment of the performance of such methods is hitherto lacking, mainly because no large-scale information on cell-specific regulatory interactions with which to evaluate CNI methods exists.

By knocking down the expression of a regulator, it is possible to determine its regulatory effect on targets by comparing the change in transcription rates with and without knock-down. Knocking down every regulator once results in a cell-specific GRN (Figure\ \ref{fig:scgrn}A) than can be used as a ground-truth for evaluating CNI methods (Figure\ \ref{fig:scgrn}B).
...
LIONESS[@kuijjer_estimatingsamplespecificregulatory_2015; @kuijjer_estimatingsamplespecificregulatory_2019] 
first infers a GRN by computing the Pearson correlation between regulators and targets for all samples and then post-processing this GRN. The possible targets for a given regulator is predefined according to prior motif analysis. To derive casewise GRNs for a particular cell $C$, the Pearson correlation is recomputed for all cells excluding $C$. The difference in Pearson correlations results in a ranking of regulatory interactions for $C$. LIONESS was excluded from this comparison as it requires motif data to be available.

SSN[@liu_personalizedcharacterizationdiseases_2016] is similar to LIONESS in methodology. To infer a casewise GRN for one sample, also the difference between the Pearson correlation between all (regulator, target) pairs is calculated for all samples and the Pearson correlation between all (regulator, target) pairs is calculated for all samples except one. SNN was excluded from this comparison as no software was provided by the authors.

SCENIC[@aibar_scenicsinglecellregulatory_2017] is a pipeline that consists of four main steps. Step 1: classical network inference is performed with arboreto, which is similar to GENIE3[@huynh-thu_inferringregulatorynetworks_2010]. Step 2: setting various cutoffs on the ranked interactions to form groups called 'regulons', each containing one regulator and multiple targets. Step 3: filter the regulons using motif analysis. Step 4: for each cell, determine an activity score of each regulon using AUCell. As a post-processing of this output, all regulon and the corresponding activity scores are combined back into a casewise GRN consisting of (cell, regulator, target, score) pairs. If multiple regulons contain the same (regulator, target) interaction, the maximum activity score is retained. For this analysis, the Python implementation of SCENIC was used, namely pySCENIC. Since dyngen does not generate motif data, step 3 in this analysis is skipped.
