...
We introduce dyngen, a multi-modality simulator of single cells and their dynamics (Figure\ \ref{fig:showcase}A).
dyngen uses Gillespie's Stochastic Simulation Algorithm (SSA)[@gillespie_exactstochasticsimulation_1977] to simulate cells at a single-molecule and single-reaction levels. This means at every time step in the simulation, a single reaction is executed, thereby modifying the abundance of the molecule counts within a cell. 
...
dyngen offers multiple significant qualities, each of which we will discuss below.
it's methodology allows keeping track of many modalities of the cell at a high resolution level
simulate different various types of dynamic processes, such as cyclic, branching, disconnected
perform time-series, perturbation, or knockdown experiments.
...
However, cells are very molecularly homogeneous, and only a fraction of all possible gene regulatory interactions is active at any point in time. Casewise network inference (CNI) methods\footnote{Other terms are commonly used when dealing with data from a particular source. For example, single-cell NI when applied to single-cell transcriptomics data; sample-specific NI when applied to bulk transcriptomics; patient-derived NI when applied to bulk profiles of patients.} tackle a more complex variant of NI, namely "Which regulators are regulating which targets in which case?" The end result of such methods is a single GRN for every case, be it a cell, tissue, patient or other sample.
...
dyngen allows deriving the ground-truth regulatory effect of a regulator on a target in a specific cell state. This information is derived by knocking down the expression of a regulator and observing the change in transcription rate as a result. Performing in silico knockdowns for every regulator in every cell results in the ground-truth casewise GRN (Figure\ \ref{fig:scgrn}A).
...
\begin{figure}[htb!]
    ...
    \textbf{A:} A cell is simulated using the global GRN (top left). However, at any particular state in the simulation, only a fraction of the gene regulatory interactions are active.
...
In eukaryotes, a gene is first transcribed to a pre-mRNA, and subsequently spliced into a mature mRNA. Both unspliced reads, which map to introns, and spliced reads, mapping to exons are often observed in RNA sequencing experiments, both in bulk[@zeisel_coupledpremrnamrna_2011] and single-cell.
